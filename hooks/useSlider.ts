...
export default function useSlider(
    ...
    vectorLeft: SVGSVGElement | null,
    vectorRight: SVGSVGElement | null,
): number {
    const dispatch = useDispatch<AppDispatch>();
    const { targetImage, projectImages } = useSelector(creationsSelector);
    ...
    const nextSlide = (): void => {
        setCountSlider((prev) => {
            ...
            requestAnimationFrame(() => rollSlider(slider.current, count));
            return count;
        });
    };
    ...
    React.useEffect(() => {
        const leftElement: SVGSVGElement | null = vectorLeft!;
        const rightElement: SVGSVGElement | null = vectorRight!;
