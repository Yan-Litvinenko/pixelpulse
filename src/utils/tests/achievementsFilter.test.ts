...
describe('filter achievements', (): void => {
    ...
    test('Should return achievements all/achieved', (): void => {
        expect(achievementsFilter(mockAchievements, 'all', 'achieved')).toEqual(
            mockAchievementsAchieved,
        );
    });

    test('Should return achievements all/in progress', (): void => {
        expect(
            achievementsFilter(mockAchievements, 'all', 'in progress'),
        ).toEqual(mockAchievementsNotAchieved);
    });

    test('Should return achievements achieved/achieved', (): void => {
        expect(
            achievementsFilter(mockAchievements, 'achieved', 'achieved'),
        ).toEqual(mockAchievementsAchieved);
    });

    test('Should return achievements achieved/in progress', (): void => {
        expect(
            achievementsFilter(mockAchievements, 'achieved', 'in progress'),
        ).toEqual([]);
    });
    ...
    test('Should return achievements in progress/in progress', (): void => {
        expect(
            achievementsFilter(mockAchievements, 'inProgress', 'in progress'),
        ).toEqual(mockAchievementsNotAchieved);
    });
});
