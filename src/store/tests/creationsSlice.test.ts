...
describe('crestionsSlice', (): void => {
    test('Should return empty state when passed an empty action', (): void => {
        const defaultState: CreationsSlice = creationsSlice.reducer(
            undefined,
            {} as { type: string },
        );
    ...
    test('Selector should return correct creationsSlice slice from the store', (): void => {
        expect(
            creationsSelector({ creations: initialState } as RootState),
        ).toEqual(initialState);
    });

    test('Should set project images', (): void => {
        const newImages: string[] = ['image-1', 'image-2', 'image-3'];
        const updateState: CreationsSlice = creationsSlice.reducer(
            initialState,
    ...
    test('Should set target image', (): void => {
        ...
        const updateState = creationsSlice.reducer(
            initialState,
            setTargetImage(newTargetImage),
        );
